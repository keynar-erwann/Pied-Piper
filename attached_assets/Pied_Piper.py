...
class MultilingualPipeyAgent(Agent):
    ...
    async def on_user_turn_completed(
        ...
        """
        # Ensure new_message is not None and has text content
        if new_message and new_message.text_content():
            rag_content = await self.my_rag_lookup(new_message.text_content())
            
            
            if rag_content:
                logger.info(f"Adding RAG content to conversation: {rag_content[:100]}...")
    ...
    async def handle_message(self, message: str):
        ...
        for pattern in song_info_patterns:
            ...
            if match:
                # Determine if it's a song and artist or just a song based on the number of groups
